...
class GenerateBlocks_Asset_Library {
	...
	public function add_menu() {
		$settings = add_submenu_page(
			'generateblocks',
			__( 'Asset Library', 'generateblocks' ),
			__( 'Asset Library', 'generateblocks' ),
			'manage_options',
		...
		if ( 'generateblocks_page_generateblocks-asset-library' === $screen->id ) {
			...
			wp_localize_script(
				...
				array(
					...
					'hasSafeSVG' => generateblocks_pro_has_safe_svg(),
					'safeSVGInstallLink' => esc_url( admin_url( 'plugin-install.php?s=safe-svg&tab=search&type=term' ) ),
				)
			);
