	...
	public function update_shape_settings( WP_REST_Request $request ) {
		...
		foreach ( $new_settings as $index => $data ) {
			...
			foreach ( $data['shapes'] as $shape_index => $shape ) {
				...
				if ( ! isset( $new_settings[ $index ]['shapes'][ $shape_index ]['id'] ) || in_array( $new_settings[ $index ]['shapes'][ $shape_index ]['id'], $usedIds ) ) {
					...
					$usedIds[] = sanitize_key( $lowercase_name . '-' . uniqid() );
				} else {
					$new_shapes[ $index ]['shapes'][ $shape_index ]['id'] = sanitize_key( $new_settings[ $index ]['shapes'][ $shape_index ]['id'] );
					$usedIds[] = sanitize_key( $new_settings[ $index ]['shapes'][ $shape_index ]['id'] );
				}
				...
				if (
					...
					method_exists( 'enshrined\svgSanitize\Sanitizer', 'minify' ) &&
					method_exists( 'enshrined\svgSanitize\Sanitizer', 'removeXMLTag' ) &&
					method_exists( 'enshrined\svgSanitize\Sanitizer', 'sanitize' )
				) {
					$sanitizer = new enshrined\svgSanitize\Sanitizer();
					$sanitizer->minify( true );
					$sanitizer->removeXMLTag( true );
					$new_shapes[ $index ]['shapes'][ $shape_index ]['shape'] = $sanitizer->sanitize( stripslashes( $shape['shape'] ) );
				} else {
		...
		return $this->success( __( 'Shapes saved.', 'generateblocks' ) );
	}
	...
	public function update_icon_settings( WP_REST_Request $request ) {
		...
		foreach ( $new_settings as $index => $data ) {
			...
			foreach ( $data['icons'] as $icon_index => $icon ) {
				// Build our unique ID if we need to.
				if ( ! isset( $new_settings[ $index ]['icons'][ $icon_index ]['id'] ) || in_array( $new_settings[ $index ]['icons'][ $icon_index ]['id'], $usedIds ) ) {
					...
					$usedIds[] = sanitize_key( $new_settings[ $index ]['icons'][ $icon_index ]['id'] );
				}
				...
				if (
					...
					$sanitizer->minify( true );
					$sanitizer->removeXMLTag( true );
					$new_icons[ $index ]['icons'][ $icon_index ]['icon'] = $sanitizer->sanitize( stripslashes( $icon['icon'] ) );
				} else {
		...
		return $this->success( __( 'Icons saved.', 'generateblocks' ) );
	}
