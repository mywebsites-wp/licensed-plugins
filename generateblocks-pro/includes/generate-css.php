...
function generateblocks_pro_generate_css( $name, $settings, $css, $desktop_css, $tablet_css, $tablet_only_css, $mobile_css ) {
			...
			if ( ! empty( $settings['advBackgrounds'] ) ) {
				foreach ( $settings['advBackgrounds'] as $key => $value ) {
					...
					if ( 'gradient' === $value['type'] ) {
						...
						$gradientColorOneValue = generateblocks_hex2rgba( $value['colorOne'], $value['colorOneOpacity'] );
						$gradientColorTwoValue = generateblocks_hex2rgba( $value['colorTwo'], $value['colorTwoOpacity'] );
		...
		if ( $settings['useBoxShadow'] && ! empty( $settings['boxShadows'] ) ) {
			foreach ( $settings['boxShadows'] as $key => $value ) {
				...
				$box_shadow = sprintf(
					...
					generateblocks_hex2rgba( $value['color'], $value['colorOpacity'] )
				);
	...
	if ( 'container' === $name || 'button-container' === $name || 'grid' === $name || 'button' === $name || 'headline' === $name ) {
		if ( 'container' === $name ) {
			$selector = '.gb-container-' . $settings['uniqueId'];
...
function generateblocks_pro_one_time_css( $css ) {
	$css .= '.gb-container-link{position:absolute;top:0;right:0;bottom:0;left:0;z-index:999;}a.gb-container{display: block;}';
